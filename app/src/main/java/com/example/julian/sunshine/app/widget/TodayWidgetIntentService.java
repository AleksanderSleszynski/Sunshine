...
public class TodayWidgetIntentService extends IntentService {
    ...
    protected void onHandleIntent(Intent intent) {
        ...
        for(int appWidgetId : appWidgetIds) {
            // Find correct layout based on the widget's width
    ...
    private int getWidgetWidth(AppWidgetManager appWidgetManager, int appWidgetId){
        return getWidgetFromOptions(appWidgetManager, appWidgetId);
    }

    @TargetApi(Build.VERSION_CODES.JELLY_BEAN)
    private int getWidgetFromOptions(AppWidgetManager appWidgetManager, int appWidgetId) {
        Bundle options = appWidgetManager.getAppWidgetOptions(appWidgetId);
        if(options.containsKey(AppWidgetManager.OPTION_APPWIDGET_MIN_WIDTH)){
            int minWidthDp = options.getInt(AppWidgetManager.OPTION_APPWIDGET_MIN_WIDTH);
            DisplayMetrics displayMetrics = getResources().getDisplayMetrics();
            return (int) TypedValue.applyDimension(TypedValue.COMPLEX_UNIT_DIP,
                    minWidthDp, displayMetrics);
        }
